...
import { ArrowDown, ArrowRight, ArrowUpRight, BriefcaseBusiness, CalendarDays, Check, ChevronLeft, ChevronRight, House, Heart, Wallet, Inbox, LayoutDashboard, LoaderCircle, Plus, Sparkles, Sun, Trash2, X } from "lucide-react";
...
function WorkspaceHome() {
  ...
  const [view, setView] = useState<"daily" | "inbox" | "finance" | "health">("daily");
  ...
  return <div className="workspace">
    ...
    <main className="main">
      <header className="topbar"><span>{view === "daily" ? <Sun size={16}/> : view === "finance" ? <Wallet size={16}/> : view === "health" ? <Heart size={16}/> : <Inbox size={16}/>} {{daily:"Daily overview",inbox:"Inbox",finance:"Finances",health:"Health"}[view]}</span><span className="save-status" role="status">{busy ? <><LoaderCircle className="spin" size={14}/> Saving…</> : error ? "Save needs attention" : loaded ? <><Check size={14}/> All changes saved</> : "Loading workspace…"}</span></header>
      <div className="page-content">
        ...
        <div className="page-heading"><div><p className="eyebrow">{selected ? date.toLocaleDateString("en-US", { weekday:"long", month:"long", day:"numeric", year:"numeric" }) : "YOUR DAILY WORKSPACE"}</p><h1>{view === "finance" ? "A clearer picture of your money." : view === "health" ? "A little attention to yourself." : view === "inbox" ? "A little room to think." : selected === today ? "A little clarity for your day." : "A little space to plan ahead."}</h1><p className="page-subtitle">{view === "finance" ? "Your personal accounts, spending, and the details worth keeping." : view === "health" ? "Your movement, rest, and measurements from Apple Health." : view === "inbox" ? "Ideas and loose ends, ready when you are." : "Your plans, priorities, and everything in between."}</p></div>{(view === "daily" || view === "inbox") && <Button id="add-item" className="primary-button" disabled={!canAct} onClick={() => start(view === "inbox" ? "note" : "priority")}><Plus size={17}/> {view === "inbox" ? "Capture a thought" : "Add to your day"}</Button>}</div>
        ...
        {view === "finance" ? <FinancePanel/> : view === "health" ? <HealthPanel/> : <><ConnectionsBar date={selected}/>
