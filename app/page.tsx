...
import { ArrowDown, ArrowRight, ArrowUpRight, BriefcaseBusiness, CalendarDays, Check, ChevronLeft, ChevronRight, House, Inbox, LayoutDashboard, LoaderCircle, Plus, Sparkles, Sun, Trash2, X } from "lucide-react";
...
function WorkspaceHome() {
  ...
  const [view, setView] = useState<"daily" | "inbox">("daily");
  ...
  return <div className="workspace">
    ...
    <main className="main">
      <header className="topbar"><span>{view === "daily" ? <Sun size={16}/> : <Inbox size={16}/>} {view === "daily" ? "Daily overview" : "Inbox"}</span><span className="save-status" role="status">{busy ? <><LoaderCircle className="spin" size={14}/> Saving…</> : error ? "Save needs attention" : loaded ? <><Check size={14}/> All changes saved</> : "Loading workspace…"}</span></header>
      <div className="page-content">
        ...
        <div className="page-heading"><div><p className="eyebrow">{selected ? date.toLocaleDateString("en-US", { weekday:"long", month:"long", day:"numeric", year:"numeric" }) : "YOUR DAILY WORKSPACE"}</p><h1>{view === "inbox" ? "A little room to think." : selected === today ? "A little clarity for your day." : "A little space to plan ahead."}</h1><p className="page-subtitle">{view === "inbox" ? "Ideas and loose ends, ready when you are." : "Your plans, priorities, and everything in between."}</p></div><Button id="add-item" className="primary-button" disabled={!canAct} onClick={() => start(view === "inbox" ? "note" : "priority")}><Plus size={17}/> {view === "inbox" ? "Capture a thought" : "Add to your day"}</Button></div>
        {area !== "all" && <div className="filter-banner">Showing {areaName(area).toLowerCase()}<button onClick={() => setArea("all")}>Show all areas <X size={14}/></button></div>}
        ...
        <div className="day-summary"><span className="summary-icon"><Check size={16}/></span><p>One thing at a time is a good pace.</p></div></div></div>
