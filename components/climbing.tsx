...
function SessionsView({ loaded, loadFailed, busy, integrationBusy, sessions, plans, removed, summary, links, events, calendarConnected, edit, editPlan, create, createPlan, schedule, openCalendar, logPlan, togglePlan, forgetLink, restore }: {
  ...
  togglePlan: (plan: ClimbingPlan) => Promise<void>; forgetLink: (link: IntegrationLink) => Promise<void>; restore: (session: ClimbingSession) => Promise<void>;
}) {
      ...
      {!loaded ? <p className="climbing-loading">Loading your plans…</p> : planned.length ? <div className="climbing-plan-list">{planned.map(plan => {
        ...
        return <article className="climbing-plan-row" key={plan.id}>
          ...
          <div className="climbing-plan-actions">
            ...
            {link && !event && <button className="climbing-quiet-action danger" disabled={integrationBusy} aria-label={`Forget unavailable Calendar link for ${plan.title}`} onClick={() => { if (window.confirm("Forget this Calendar link? The Calendar event, if it still exists, will not be changed. You can then create a replacement link.")) void forgetLink(link); }}>Forget Calendar link</button>}
