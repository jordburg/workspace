...
export const healthSnapshotSchema=z.object({version:z.literal(1),id:z.string().uuid(),generatedAt:timestamp,timeZone:z.string().max(100).refine(value=>{try{new Intl.DateTimeFormat("en",{timeZone:value});return true;}catch{return false;}}),from:daySchema,to:daySchema,days:z.array(healthDaySchema).min(1).max(92),workouts:z.array(z.object({id:z.string().uuid(),name:z.string().max(100),start:timestamp,end:timestamp,minutes:z.number().finite().nonnegative().max(10080)}).strict().refine(w=>new Date(w.end)>=new Date(w.start))).max(2000)}).strict().superRefine((s,ctx)=>{
