...
Open **Settings → Companion devices and Apple Health**. The native project and installation guide are in [`companion/README.md`](companion/README.md). On iPhone, Workspace is a focused companion with five tabs: **Today**, **Capture**, **Tasks**, **Climbing**, and **Lesson**. On iPad, it uses a desktop-style sidebar and adds wider views while the Mac is available. Both retain protected snapshots for the core planning, Climbing, and Chess experience. The Mac remains the source of truth: provider credentials, Plaid Link, and Google/Todoist connection setup stay on the Mac, while paired devices read sanitized views and send revision-checked edits through the same local services.

The app connects only while the Mac Workspace is running on the same private Wi-Fi. Its dedicated HTTPS listener uses a certificate pinned during pairing and a separate bearer token stored in each device’s Keychain. The desktop web server, provider credentials, and private data files remain loopback-only. Every Workspace token can use the shared planning, capture, Todoist, Calendar, Climbing, and current-lesson routes. An iPad token can also use the explicit Mail, Finance, Writing, Chess-review, and read-only Health routes. The single paired iPhone owns Apple Health sync: only its token can upload Apple Health data, receive pending Health commands, or confirm Health writes. Existing Health-only iPhone tokens continue to sync Health but cannot read the other companion areas. Download the matching fresh iPhone or iPad pairing file for each device.

The iPhone companion supports a daily overview with a compact Health summary, quick capture to Workspace Captures, the selected Todoist Personal task list, Climbing records, and the current in-progress Chess lesson. It is designed to spend most of the day disconnected: protected planning, task, climbing, and lesson snapshots open normally without a connection warning on every tab. Today shows one quiet planning-freshness line, while pairing, exact sync status, last Mac contact, manual sync, Apple Health permissions and history import, pending Health write approvals, and climbing links live in Settings. The iPad can also browse Mail, Finance, Writing, the Chess catalog and review queue, and detailed Health views through its desktop-style layout while the Mac is reachable. Those wider views are fetched on demand and are not retained in the offline snapshot. Apple Health collection and writes remain assigned to the iPhone; the iPad reads the Mac’s saved summary and cannot duplicate Health entries.

This Mac app cannot access HealthKit directly; Workspace on iPhone uses HealthKit with your selected permissions.
...
Companion sync is **disabled by default**. Enabling it starts a separate HTTPS listener on the selected private Wi-Fi IP, normally port 5174. It exposes only an allowlist of versioned operations selected for the calling device class. iPhone integration responses omit Gmail state, messages, and Gmail relationship changes; iPad Mail receives only the public Gmail view and never provider credentials. The browser application and ordinary data APIs remain on loopback. No firewall, privacy, or device settings are changed automatically. Pairing files expire after 10 minutes, are bound to either iPhone or iPad access, pair one device, and contain a temporary credential; an interrupted exchange can safely replay the same result during a short retry window. Delete each transferred file after pairing. A successful pairing adds that device without revoking existing devices, up to eight devices total with one Health-owning iPhone. Pairing the same device with the same Mac rotates only that device’s credential. Unpairing in the native app revokes only that device. Disabling companion sync revokes every device immediately.

The native client permits only HTTPS on private IPv4 addresses, validates the paired certificate fingerprint, trusts that certificate only for its session, checks its SAN/expiry, and refuses redirects. App Transport Security has no insecure HTTP exceptions; the connection requires TLS 1.2 or later. Device credentials and iPhone Health receipts use non-synchronizing, unlocked-device-only Keychain storage. No iCloud or hosted Workspace transport is used. The Mac certificate is valid for one year; disable/re-enable and re-pair each device to renew it or change the Wi-Fi IP.

The Mac must be awake with Workspace running, and a companion device must be unlocked on the same private Wi-Fi. Sync runs on demand and makes one short attempt when the companion opens; expected offline failures remain quiet and automatic retries back off. A USB cable does not currently carry Workspace app data. Snapshots, pending entries, and the bounded set of device token hashes are stored in `~/Data/personal-workspace/health.private.json`; Health receipts stay in the iPhone Keychain, and the local TLS key/certificate are alongside the Mac state with owner-only permissions. Reopening Workspace resumes a previously enabled listener. Disabling pairing retains the saved snapshot and entry history.
