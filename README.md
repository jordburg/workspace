...
Open **Health → iPhone setup**. The native project and installation guide are in [`companion/README.md`](companion/README.md). This Mac app cannot access HealthKit directly; its iPhone companion uses HealthKit with your selected permissions.
...
Health sync is **disabled by default**. Enabling it starts a separate, health-only HTTPS listener on the selected private Wi-Fi IP, port 5174. The browser application, finances, and existing integrations remain on loopback. No firewall, privacy, or device settings are changed automatically. Pairing files expire after 10 minutes, work once, and contain a temporary credential; delete the transferred file after pairing. A new successful pairing revokes the previous device token. Disabling sync also revokes pairing immediately.

The companion permits only HTTPS on private IPv4 addresses, validates the paired certificate fingerprint, trusts that certificate only for its session, checks its SAN/expiry, and refuses redirects. Private network ATS exceptions enable this custom trust path; the application still requires TLS 1.2 or later. Device credentials/receipts use non-synchronizing, unlocked-device-only Keychain storage. No iCloud or hosted health transport is used. The Mac certificate is valid for one year; disable/re-enable and re-pair to renew it or change the Wi-Fi IP.
...
Install/run the updated companion, review its additional Health permissions, and tap **Sync now** for recent sleep. **Import sleep history** sends older records in complete 30-day batches from a chosen date (default January 2020); keep the companion open and the Mac awake. Stop import keeps completed ranges. A retry starts at the selected date and safely replaces those ranges. The existing pairing stays valid.
