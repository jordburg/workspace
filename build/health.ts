  ...
  async function handle(req:IncomingMessage,res:ServerResponse,next?:()=>void){
    ...
    try{
      ...
      const result=await exclusive(async()=>{const s=await read();
        ...
        if(path==="/disable"){s.enabled=false;delete s.tokenHash;delete s.pairing;await write(s);await stop();runtimeError=null;return view(s);}
        if(path==="/pairing"){
          if(!s.enabled||!server?.listening||!endpoint)throw new HealthError("Enable the iPhone connection first.");const code=randomBytes(32).toString("base64url");s.pairing={hash:hash(code),expires:Date.now()+600000};await write(s);const cert=new X509Certificate(await readFile(certificate));return {version:1,type:"personal-workspace-health-pairing",url:endpoint,fingerprint:hashBuffer(cert.raw),code,expiresAt:new Date(s.pairing.expires).toISOString()};
        }
        if(path==="/weight"){
