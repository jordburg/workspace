...
async function harness() {
  ...
  const remote={calls:[],task:{...baseTask},event:structuredClone(baseEvent),failTasks:false,failEvents:false,failGmail:false,failSendUncertain:false,failModifyUncertain:false,failTrashUncertain:false,gmailMetadataDelay:0,gmailMetadataActive:0,gmailMetadataMax:0,missingGmailIds:new Set(),projects:[{id:'personal',name:'Personal'},{id:'learning',name:'Learning',parent_id:'personal'},{id:'work',name:'Artek'},{id:'work-child',name:'Personal',parent_id:'work'}],pagination:false,primaryId:PERSONAL_CALENDAR,gmailProfile:PERSONAL_CALENDAR,gmailMessages:[mail('personal'),mail('personal-two'),mail('header-work',{payload:{headers:[{name:'From',value:'Colleague <person@artek.energy>'},{name:'To',value:PERSONAL_CALENDAR},{name:'Subject',value:'Work mail'}]}}),mail('label-work',{labelIds:['INBOX','UNREAD','Label_Work_Artek']})],tokenExchanges:[],commands:new Map(),googleWrites:[],gmailWrites:[],gmailSends:[],todoistWrites:0,mutationDelay:0};
  ...
  const fakeFetch=async(input,options={})=>{
    ...
    if(url.hostname==='gmail.googleapis.com') {
      ...
      if(url.pathname.endsWith('/messages/send')&&method==='POST'){const body=JSON.parse(options.body);remote.gmailSends.push(body);const mime=Buffer.from(body.raw,'base64url').toString('utf8');const messageId=/^Message-ID: (.+)$/m.exec(mime)?.[1].trim()||'';const requestHash=/^X-Workspace-Request-Hash: (.+)$/m.exec(mime)?.[1].trim()||'';const sentId=`sent-${remote.gmailSends.length}`;remote.gmailMessages.push(mail(sentId,{threadId:body.threadId||`sent-thread-${remote.gmailSends.length}`,labelIds:['SENT'],historyId:randomUUID(),payload:{headers:[{name:'From',value:PERSONAL_CALENDAR},{name:'To',value:'friend@example.com'},{name:'Subject',value:'Sent message'},{name:'Message-ID',value:messageId},{name:'X-Workspace-Request-Hash',value:requestHash}]}}));if(remote.failSendUncertain)throw new Error('connection dropped after delivery');return json({id:sentId,threadId:body.threadId||`sent-thread-${remote.gmailSends.length}`});}
...
test('Gmail OAuth, personal boundary, bounded cache, and independent disconnect',async t=>{
  ...
  await t.test('inspects at most 100 newest IDs and skips a message that disappears mid-sync',async()=>{
    const h=await harness();try{
...
test('Gmail explicit send and reply use safe text MIME and durable request receipts',async()=>{
  const h=await harness();try{
    ...
    const requestId=randomUUID();const outbound={requestId,to:['friend@example.com'],cc:[],bcc:[],subject:'Café plans',body:'First line\nSecond line',confirm:true};const sent=await h.post('/gmail/send',outbound);assert.equal(sent.status,200);assert.equal(h.remote.gmailSends.length,1);const decoded=Buffer.from(h.remote.gmailSends[0].raw,'base64url').toString('utf8');assert.match(decoded,/^To: friend@example\.com\r\n/);assert.match(decoded,/Subject: =\?UTF-8\?B\?/);assert.match(decoded,new RegExp(`Message-ID: <workspace\\.${requestId}@jordmburg-workspace\\.local>`));assert.match(decoded,/Content-Type: text\/plain; charset=UTF-8/);assert.equal(decoded.includes('fake-client-secret'),false);assert.equal(decoded.includes('test-gmail-access'),false);
    ...
    const droppedId=randomUUID();const dropped={requestId:droppedId,to:['friend@example.com'],subject:'Connection dropped',body:'Maybe sent once',confirm:true};h.remote.failSendUncertain=true;const droppedResult=await h.post('/gmail/send',dropped);assert.equal(droppedResult.status,409);assert.match(droppedResult.data.error,/may already have sent.*Check Sent/i);const afterDrop=h.remote.gmailSends.length;h.remote.failSendUncertain=false;const blockedRetry=await h.post('/gmail/send',dropped);assert.equal(blockedRetry.status,409);assert.equal(h.remote.gmailSends.length,afterDrop);assert.equal(JSON.parse(await readFile(join(h.directory,'integrations.private.json'),'utf8')).receipts[droppedId].provider,'gmail-send-pending');
