...
test('linked provider creates retain exact remote identities',async t=>{
  await t.test('Todoist goal task requires an explicit unlink before replacement even when its task is absent from the cache',async()=>{
    const h=await harness();try{
      ...
      const created=await h.post('/mutate/planning',mutation);assert.equal(created.status,200);assert.equal(created.data.links.length,1);
      ...
      const writes=h.remote.todoistWrites;const replay=await h.post('/mutate/planning',mutation);assert.equal(replay.status,200);assert.equal(h.remote.todoistWrites,writes);assert.equal(replay.data.links[0].id,link.id);
      ...
      const detached=await h.post('/unlink/planning',{id:link.id});assert.equal(detached.status,200);assert.deepEqual(detached.data.links,[]);assert.equal(h.remote.todoistWrites,writes);
...
test('Gmail follow-up relations are durable, typed, and reconciled only through their target provider',async()=>{
  const h=await harness();try{
    ...
    const writesBeforeBlockedCreate=h.remote.todoistWrites;const blockedCreate=await h.post('/mutate/planning',input);assert.equal(blockedCreate.status,403);assert.equal(h.remote.todoistWrites,writesBeforeBlockedCreate);assert.deepEqual((await h.view()).gmail.relations,[]);
    ...
    const blockedUnlink=await h.post('/unlink/planning',{id:relation.id});assert.equal(blockedUnlink.status,403);assert.equal((await h.view()).gmail.relations.some(item=>item.id===relation.id),true);
